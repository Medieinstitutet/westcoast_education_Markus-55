...
const bookCourse = async (
  ...
  const user = users.find((user: User) => user);
  ...
  const userEmailData: User[] = await userEmailResponse.json();

  // Check if the user data is found
  if(userEmailData.length > 0) {
    const user = userEmailData[0];

    const isAlreadyBooked = user.booking.some((booking: BookingInfo) => {
      return(
    ...
    if(!isAlreadyBooked) {
      if(user) {
        const hasBookedOnlineAndClassroom = user.booking.some((booking: BookingInfo) => {
          return(
            booking.courseId === course.id &&
        ...
        const hasBookedOnline = user.booking.some((booking: BookingInfo) => {
          return(
            booking.courseId === course.id &&
        ...
        const hasBookedClassroom = user.booking.some((booking: BookingInfo) => {
          return(
            booking.courseId === course.id &&
